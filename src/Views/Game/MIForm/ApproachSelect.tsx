...
export const ApproachSelect = ({ $, dicePool$ }: { $: FunState<Approach$>; dicePool$: FunState<DicePoolState> }) => {
  ...
  return (
    <div className={styles.Approach}>
      <label className={isActive ? styles.active : ''}>Approach</label>
      <TierSelect $={$.prop('tier')} />
      <button className={classes(styles.approachButton, isActive && styles.active)} onClick={setOpen.bind(null, true)}>
        {approach || 'Select'}
      </button>
      {open && (
